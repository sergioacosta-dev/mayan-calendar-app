...
def show_clock():
    ...
    def draw_dot(angle, r, c, s, tag=""):
        a=math.radians(angle)
        x=CENTER_X+math.sin(a)*r; y=CENTER_Y-math.cos(a)*r
        canvas.create_oval(x-s, y-s, x+s, y+s, fill=c, outline="", tags=tag)

    def draw_face():
        canvas.delete("face")
        for _, cnt, r, _, _ in RINGS:
            for i in range(cnt):
                draw_dot(i*(360/cnt), r, "grey30", 3, "face")

    def draw_legend():
        LEGEND=[("Seconds","red"),("Minutes","orange"),("Hours","yellow"),
                ("Weekdays","green"),("MonthDays","blue"),("Months","indigo"),
                ("Years","purple")]
        ly=CENTER_Y+CLOCK_RADIUS+280; sp=100
        sx=CENTER_X-sp*(len(LEGEND)//2)
        for i,(lbl,col) in enumerate(LEGEND):
            x=sx+i*sp
            canvas.create_oval(x-6,ly-6,x+6,ly+6, fill=col, outline="")
            canvas.create_text(x, ly+20, text=lbl, fill="white", font=("Helvetica",10), tags="face")

    def update_clock():
        canvas.delete("hands")
        now=time.localtime(); moon=get_today_moon_data()
        vals={
            "Seconds": now.tm_sec,
            "Minutes": now.tm_min+now.tm_sec/60,
            "Hours": (now.tm_hour%24)+now.tm_min/60,
            "Weekdays": now.tm_wday,
            "MonthDays": (now.tm_mday-1)%28,
            "Months": moon["moon_number"]-1,
            "Years": now.tm_year%25920
        }
        for name,cnt,r,size,color in RINGS:
            f=vals[name]/cnt
            ang=(1-f)*360 if name in ("Seconds","Minutes","Hours") else f*360
            draw_dot(ang, r, color, size, "hands")
